...
export default function AdminShell({ children, title, description, requireAdmin = false }: { children: React.ReactNode; title: string; description: string; requireAdmin?: boolean }) {
  ...
  if (requireAdmin && !["admin", "super_admin"].includes(user.role)) return <div className="grid min-h-screen place-items-center bg-[#f6f1e7] p-6 text-center" dir="rtl"><div><p className="text-xs font-bold tracking-[.16em] text-[#af7c42]">وصول محمي</p><h1 className="mt-3 font-serif text-4xl text-[#1d2940]">تحتاج إلى صلاحية مدير</h1><p className="mt-3 max-w-md text-sm leading-7 text-[#667085]">إدارة حسابات المستخدمين مخصصة للمديرين ومديري النظام فقط.</p><Link href="/admin"><Button className="mt-7 bg-[#1d2940]">العودة إلى الإدارة</Button></Link></div></div>;
