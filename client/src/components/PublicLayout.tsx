...
export default function PublicLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f6f1e7] text-[#1d2940]" dir="rtl">
      <header className="sticky top-0 z-40 border-b border-[#1d2940]/10 bg-[#f6f1e7]/95 backdrop-blur-md">
        <div className="container flex h-[74px] items-center justify-between gap-5">
          <Link href="/" className="group flex items-center gap-3 text-[#1d2940]">
            <span className="grid h-10 w-10 place-items-center rounded-full bg-[#1d2940] font-serif text-xl text-[#f6f1e7] transition-transform duration-200 group-hover:rotate-6">ر</span>
            <span>
          ...
          <div className="hidden items-center gap-2 md:flex">
            ...
            {isAuthenticated ? (
              <Button variant="outline" className="border-[#1d2940]/20 bg-transparent" onClick={() => navigate(user?.role !== "user" ? "/admin" : "/library")}>مكتبتي</Button>
            ) : (
              <Button className="bg-[#1d2940] text-[#f6f1e7] hover:bg-[#33435e]" onClick={() => startLogin()}>دخول</Button>
            )}
          </div>
        ...
        {menuOpen && (
          <div className="border-t border-[#1d2940]/10 bg-[#f6f1e7] px-5 py-4 md:hidden">
            <nav className="grid gap-1" aria-label="التنقل المحمول">
              {[...links, { href: "/search", label: "البحث" }].map(link => <Link key={link.href} href={link.href} className="rounded-lg px-3 py-3 font-semibold hover:bg-[#e9dfca]" onClick={() => setMenuOpen(false)}>{link.label}</Link>)}
            </nav>
          </div>
        )}
      </header>
