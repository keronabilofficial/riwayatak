...
export default function Plans() {
  ...
  return <PublicLayout>
    <section className="border-b border-border bg-[radial-gradient(circle_at_top,rgba(175,124,66,0.16),transparent_42%)] py-16 md:py-24">
      <div className="container text-center">
        <p className="text-sm font-bold text-primary">عضوية القراءة</p>
        <h1 className="mt-4 font-serif text-4xl leading-tight md:text-6xl">اقرأ أكثر، واستمع على مهل</h1>
        <p className="mx-auto mt-5 max-w-2xl leading-8 text-muted-foreground">ابدأ دائمًا بالفصلين الأولين مجانًا من كل رواية، ثم اختر الباقة التي تناسب وتيرة قراءتك. التقييمات والمراجعات متاحة للجميع ومستقلة عن الدفع.</p>
    ...
    <section className="container py-12 md:py-16">
      {isLoading ? <div className="grid min-h-80 place-items-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : <div className="grid gap-5 lg:grid-cols-4">{groupedOptions.map(group => {
        ...
        return <article key={group.planName} className={`flex flex-col rounded-3xl border p-6 transition-shadow ${group.planName === "ultra" ? "border-primary bg-primary/5 shadow-lg shadow-primary/10" : "border-border bg-card"}`}>
          <div className="flex items-center justify-between"><h2 className="font-serif text-3xl">{planLabels[group.planName]}</h2>{group.planName === "ultra" && <span className="rounded-full bg-primary px-3 py-1 text-xs font-bold text-primary-foreground">الأوسع</span>}</div>
      ...
      <section className="mx-auto mt-12 max-w-xl rounded-3xl border border-border bg-card p-6 md:p-8">
        <div className="flex items-center gap-3"><LockKeyhole className="h-5 w-5 text-primary" /><div><h2 className="font-serif text-2xl">إتمام الاشتراك</h2><p className="mt-1 text-sm text-muted-foreground">يُحوَّل الدفع إلى صفحة Paymob الآمنة.</p></div></div>
