...
export default function Home() {
  ...
  const { data, isLoading } = trpc.catalog.home.useQuery({ languageCode: language });
  const { user } = useAuth();
  ...
  return <PublicLayout>
      ...
      <div className="container grid min-h-[570px] items-center py-20 md:py-28">
        <div className="max-w-2xl">
          ...
          <form onSubmit={submitSearch} className="mt-9 flex max-w-lg items-center gap-2 rounded-2xl bg-white p-2 shadow-2xl shadow-black/20">
            <Search className="mr-2 h-5 w-5 text-[#7d8796]" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder={text.searchPlaceholder} className="h-11 border-0 bg-transparent text-[#1d2940] shadow-none focus-visible:ring-0" /><Button type="submit" className="h-11 bg-[#af7c42] px-6 text-white hover:bg-[#936536]">{text.searchAction}</Button>
          </form>
          <div className="mt-8 flex flex-wrap items-center gap-4 text-sm text-[#f6f1e7]/65"><span className="inline-flex items-center gap-2"><BookOpen className="h-4 w-4 text-[#d5a85e]" />{text.readWithoutDistraction}</span><span>{text.originalArabicExperience}</span>{continueReading?.[0] ? <Link href={`/read/${continueReading[0].novelSlug}/${continueReading[0].chapterSlug}`} className="inline-flex items-center gap-2 rounded-xl bg-[#d5a85e] px-4 py-2 font-bold text-[#1d2940] transition hover:bg-[#ead7ad]"><BookOpen className="h-4 w-4" />{text.continueStory}<span className="text-xs font-normal opacity-75">{continueReading[0].novelTitle} · {continueReading[0].progressPercent}%</span></Link> : null}</div>
        </div>
      </div>
    </section>

    <section className="container py-18 md:py-24">
      <SectionHeading eyebrow={text.latest} title={text.continueStory} href="/novels" />
    ...
    {user ? <section className="container pb-18 md:pb-24"><div className="rounded-[1.5rem] border border-[#1d2940]/10 bg-[#fbf8f2] p-6 md:p-8"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><History className="h-5 w-5 text-[#af7c42]" /><div><p className="text-xs font-bold text-[#af7c42]">{text.readingHistory}</p><h2 className="mt-1 font-serif text-3xl text-[#1d2940]">{text.readingHistory}</h2></div></div><Link href="/library" className="text-sm font-bold text-[#af7c42] hover:text-[#936536]">{text.library}</Link></div>{continueReading?.length ? <div className="mt-5 grid gap-3 md:grid-cols-3">{continueReading.slice(0, 3).map(item => <Link key={`${item.novelId}-${item.chapterId}`} href={`/read/${item.novelSlug}/${item.chapterSlug}`} className="group rounded-2xl border border-[#1d2940]/10 bg-white p-4 transition hover:-translate-y-0.5 hover:border-[#af7c42]/50"><strong className="block truncate font-serif text-lg text-[#1d2940] group-hover:text-[#af7c42]">{item.novelTitle}</strong><span className="mt-1 block truncate text-xs text-[#667085]">{item.chapterTitle}</span><div className="mt-3 flex items-center justify-between text-xs text-[#667085]"><span>{item.progressPercent}{text.progressComplete}</span><BookOpen className="h-4 w-4 text-[#af7c42]" /></div></Link>)}</div> : <p className="mt-5 rounded-xl bg-white p-4 text-sm text-[#667085]">{text.noHistory}</p>}</div></section> : null}

    {user ? <section className="container pb-18 md:pb-24"><div className="rounded-[1.5rem] border border-[#af7c42]/25 bg-[#af7c42]/5 p-6 md:p-8"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><Sparkles className="h-5 w-5 text-[#af7c42]" /><div><p className="text-xs font-bold text-[#af7c42]">{text.personalizedChoices}</p><h2 className="mt-1 font-serif text-3xl text-[#1d2940]">{text.smartRecommendations}</h2></div></div><Link href="/library" className="text-sm font-bold text-[#af7c42] hover:text-[#936536]">{text.managePreferences}</Link></div>{suggestions?.length ? <div className="mt-5 grid gap-3 md:grid-cols-3">{suggestions.slice(0, 6).map(item => <Link key={item.novelId} href={`/novels/${item.slug}`} className="group rounded-2xl border border-[#1d2940]/10 bg-white p-4 transition hover:-translate-y-0.5 hover:border-[#af7c42]/50"><strong className="block truncate font-serif text-lg text-[#1d2940] group-hover:text-[#af7c42]">{item.title}</strong><span className="mt-1 block truncate text-xs text-[#667085]">{item.authorName}</span><p className="mt-2 line-clamp-2 text-xs leading-6 text-[#667085]">{item.shortDescription || "رواية تتقاطع مع ذائقتك القرائية."}</p><span className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-[#af7c42]"><Sparkles className="h-3.5 w-3.5" />{text.suggestionBased}</span></Link>)}</div> : <p className="mt-5 rounded-xl bg-white p-4 text-sm text-[#667085]">{text.startReadingForSuggestions}</p>}</div></section> : null}

    <section className="border-y border-[#1d2940]/10 bg-[#e9dfca]/45 py-18 md:py-24"><div className="container"><SectionHeading eyebrow={text.readingGuide} title={text.chooseMood} href="/categories" />
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{categories.length ? categories.slice(0, 8).map((category, index) => <Link key={category.id} href={`/categories/${category.slug}`} className="group flex min-h-32 items-end justify-between rounded-2xl border border-[#1d2940]/10 bg-[#f6f1e7] p-5 transition hover:-translate-y-1 hover:border-[#af7c42]/50 hover:shadow-xl hover:shadow-[#1d2940]/5"><span><span className="mb-2 block text-xs font-bold text-[#af7c42]">{String(index + 1).padStart(2, "0")}</span><strong className="font-serif text-2xl">{category.name}</strong><span className="mt-1 block text-xs text-[#667085]">{category.description || text.discoverNewWorlds}</span></span><ArrowLeft className="h-5 w-5 text-[#af7c42]" /></Link>) : <div className="col-span-full rounded-2xl bg-[#f6f1e7] p-7 text-sm text-[#667085]">{text.manageContentDescription}</div>}</div>
    </div></section>

    <section className="container py-18 md:py-24"><SectionHeading eyebrow={text.latest} title={text.continueStory} href="/novels" />
      {latest.length > 0 ? <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">{latest.slice(0, 6).map(novel => <BookCard key={novel.id} novel={novel} featured />)}</div> : canManageContent ? <div className="rounded-[1.5rem] bg-[#1d2940] p-9 text-[#f6f1e7]"><p className="text-sm text-[#d5a85e]">{text.centralContentManagement}</p><h3 className="mt-2 font-serif text-3xl">{text.buildLibrary}</h3><p className="mt-3 max-w-xl text-sm leading-7 text-[#f6f1e7]/65">{text.manageContentDescription}</p><Link href="/admin"><Button className="mt-6 bg-[#d5a85e] text-[#1d2940] hover:bg-[#ead7ad]">{text.goToAdmin}</Button></Link></div> : <div className="rounded-[1.5rem] border border-dashed border-[#af7c42]/40 bg-[#fbf8f2] p-9 text-center"><p className="font-serif text-3xl text-[#1d2940]">{text.newStoriesStartHere}</p><p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-[#667085]">{text.noPublishedNovels}</p><Link href="/novels"><Button className="mt-6 bg-[#af7c42] text-white hover:bg-[#936536]">{text.exploreNovels}</Button></Link></div>}
    </section>
  </PublicLayout>;
}
