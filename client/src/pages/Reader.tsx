...
export default function Reader({ novelSlug, chapterSlug }: { novelSlug: string; chapterSlug: string }) {
  ...
  const { data: chapter, isLoading, isFetching } = trpc.catalog.read.useQuery({ novelSlug, chapterSlug, languageCode: language });
  ...
  const suggestTranslation = trpc.language.suggestChapterTranslation.useMutation({ onSuccess: () => { setSuggestionSource(""); setSuggestionText(""); setSuggestionNote(""); } });
  const saveOffline = () => {
  ...
  return <div className="min-h-screen bg-background text-foreground" dir={language === "ar" ? "rtl" : "ltr"}>
    {isFetching ? <div className="fixed inset-x-0 top-0 z-50 h-1 overflow-hidden bg-[#af7c42]/20" role="status" aria-label="جارٍ تحميل الفصل"><div className="h-full w-1/3 animate-pulse bg-[#af7c42]" /></div> : null}
    ...
    <main className="mx-auto max-w-4xl px-5 py-8 md:py-12">
      ...
      <div className="mx-auto max-w-2xl">
        ...
        {!readableChapter.access.allowed ? <section className="mx-auto mt-10 max-w-xl rounded-3xl border border-[#af7c42]/30 bg-card p-7 text-center shadow-sm"><LockKeyhole className="mx-auto h-7 w-7 text-[#af7c42]" /><h2 className="mt-4 font-serif text-2xl">هذا الفصل متاح للمشتركين</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">{readableChapter.access.reason}</p><Link href="/plans" className="mt-5 inline-flex rounded-xl bg-[#af7c42] px-5 py-3 text-sm font-bold text-white">عرض الباقات</Link></section> : <>
          ...
          {selectedQuote ? <div className="mt-8 flex flex-wrap items-center gap-3 rounded-2xl border border-[#af7c42]/35 bg-[#af7c42]/10 px-4 py-3 text-sm" role="status"><Quote className="h-4 w-4 shrink-0 text-[#af7c42]" /><p className="min-w-0 flex-1 truncate">«{selectedQuote.text}»</p><Button type="button" size="sm" className="gap-1 bg-[#af7c42] text-white hover:bg-[#936536]" disabled={saveQuote.isPending} onClick={() => saveQuote.mutate({ novelId: readableChapter.novelId, chapterId: readableChapter.chapterId, selectedText: selectedQuote.text })}>{saveQuote.isPending ? "جارٍ الحفظ…" : <><BookmarkPlus className="h-4 w-4" />حفظ الاقتباس</>}</Button><Button type="button" variant="ghost" size="icon" className="h-8 w-8" aria-label="إلغاء تحديد الاقتباس" onClick={() => setSelectedQuote(null)}><X className="h-4 w-4" /></Button></div> : null}<article ref={articleRef} onMouseUp={handleTextSelection} onTouchEnd={handleTextSelection} className="mt-12 select-text font-serif leading-[2.35]" style={{ fontSize: `${fontScale}rem` }}>{paragraphs.map((paragraph, index) => <p key={index} className="mb-8">{paragraph}</p>)}</article>{isAuthenticated && language !== "ar" && readableChapter.access.allowed ? <section className="mt-12 rounded-2xl border border-[#af7c42]/25 bg-[#af7c42]/5 p-5"><h2 className="font-serif text-xl">اقتراح تعديل للترجمة</h2><p className="mt-2 text-sm leading-7 text-muted-foreground">إذا لاحظت عبارة غير دقيقة، أرسل النص الحالي واقتراحك. سيُراجع الاقتراح قبل اعتماده.</p><div className="mt-4 grid gap-3"><textarea value={suggestionSource} onChange={event => setSuggestionSource(event.target.value)} placeholder="النص الحالي في الترجمة" className="min-h-24 rounded-xl border border-border bg-background p-3 text-sm" maxLength={4000} /><textarea value={suggestionText} onChange={event => setSuggestionText(event.target.value)} placeholder="اقتراحك البديل" className="min-h-24 rounded-xl border border-border bg-background p-3 text-sm" maxLength={4000} /><input value={suggestionNote} onChange={event => setSuggestionNote(event.target.value)} placeholder="ملاحظة اختيارية" className="h-10 rounded-xl border border-border bg-background px-3 text-sm" maxLength={500} /><Button type="button" className="w-fit bg-[#af7c42] text-white hover:bg-[#936536]" disabled={suggestTranslation.isPending || !suggestionSource.trim() || !suggestionText.trim()} onClick={() => suggestTranslation.mutate({ chapterId: readableChapter.chapterId, languageCode: language, sourceText: suggestionSource, suggestedText: suggestionText, note: suggestionNote || undefined })}>{suggestTranslation.isPending ? "جارٍ إرسال الاقتراح..." : "إرسال الاقتراح للمراجعة"}</Button>{suggestTranslation.isSuccess ? <p className="text-sm text-emerald-600">تم إرسال اقتراحك للمراجعة، شكرًا لمساهمتك.</p> : null}{suggestTranslation.error ? <p className="text-sm text-destructive">تعذر إرسال الاقتراح حاليًا. حاول مرة أخرى.</p> : null}</div></section> : null}{isAuthenticated ? <section className="mt-12 rounded-2xl border border-current/10 bg-current/5 p-5"><div className="flex items-center gap-2"><Quote className="h-4 w-4 text-[#af7c42]" /><h2 className="font-serif text-xl">اقتباساتك المحفوظة</h2><span className="text-xs opacity-60">({quotes?.length ?? 0})</span></div>{quotes?.length ? <div className="mt-4 grid gap-3">{quotes.map(item => <div key={item.id} className="rounded-xl border border-current/10 bg-background/60 p-4"><blockquote className="border-r-2 border-[#af7c42] pr-3 text-sm leading-7">«{item.selectedText}»</blockquote><div className="mt-3 flex items-center justify-between gap-2"><small className="opacity-60">حُفظ في {new Date(item.createdAt).toLocaleDateString("ar-EG")}</small><Button type="button" variant="ghost" size="sm" className="h-8 text-destructive" disabled={deleteQuote.isPending} onClick={() => deleteQuote.mutate({ id: item.id })}><Trash2 className="ml-1 h-3.5 w-3.5" />حذف</Button></div></div>)}</div> : <p className="mt-3 text-sm leading-7 opacity-65">حدد جملة من النص ثم اختر «حفظ الاقتباس» لتظهر هنا وتظل خاصة بحسابك.</p>}</section> : null}<ChapterComments chapterId={readableChapter.chapterId} />
        </>}
